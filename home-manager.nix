...
let
  ...
  trimOptionalPrefix = persistentStorage: path:
    if persistentStorage.removePrefixDirectory then joinPathComponents (tail (splitPath [ path ]))
    else path;
    ...
    let
      dirs = map
        (dir: {
          ...
          target = trimOptionalPrefix persistentStorage (getDirPath dir);
          type = "directory";
      ...
      files = map
        (file: {
          persistence = persistentStorage.persistentStoragePath;
          target = trimOptionalPrefix persistentStorage file;
          type = "file";
          method = "symlink";
          allowOther = persistentStorage.allowOther;
        })
...
in
{
  ...
  config = {
    home.file =
      let
        ...
        mkLinkNameValuePair = opt: {
          name = opt.target;
          value = { source = link (mergePaths [ opt.persistence opt.target ]); };
        };
      in
    ...
    systemd.user.services =
      let
        mkTargetDir = opt: escapeShellArg (mergePaths [ opt.persistence opt.target ]);
        mkMountPoint = opt: escapeShellArg (mergePaths [ config.home.homeDirectory opt.target ]);
    ...
    home.activation =
      let
        ...
        mkBindMount = opt:
          let
            mountDir = opt.target;
            targetDir = escapeShellArg (mergePaths [ opt.persistence opt.target ]);
            mountPoint = escapeShellArg (mergePaths [ config.home.homeDirectory mountDir ]);
            bindfsOptions = concatStringsSep "," (
              optional (!opt.allowOther) "no-allow-other"
              ++ optional (versionAtLeast pkgs.bindfs.version "1.14.9") "fsname=${targetDir}"
            );
        ...
        (mkIf (any (opt: opt.type == "file" || opt.method == "symlink") persistenceOptions) {
          createTargetFileDirectories =
            dag.entryBefore
              ...
              (concatMapStrings
                (opt: ''
                  mkdir -p ${escapeShellArg (mergePaths [ opt.persistence (dirOf opt.target) ])}
                '')
                persistenceOptions);
        })
      ];
